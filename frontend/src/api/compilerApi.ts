...
export const compileCircuit = async (
  ...
  const response = await axios.post(`${API_BASE}/compile`, payload, {
    timeout: 30000,
    signal,
  });
...
export const compileFromGraph = async (
  nodes: Node[],
  edges: Edge[],
  signal?: AbortSignal,
): Promise<CompileResult> => {
